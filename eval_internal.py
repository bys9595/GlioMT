...
class Brain_Dataset(Dataset):
    def __init__(self, train_val_test, cfg, transform=None):
        ...
        # Train / Val
        _, _, val_name_list, val_cls_list = self.split_class_balanced_data(img_name_list, cls_list, self.k_fold, self.cfg.data.fold_num)     
    ...
    def patient_and_class_selection(self):
        ...
        if self.cls_mode == 'idh':
            ...
            cls_list = np.array(df_from_excel['IDH_mutation']) # IDH mutation status 0/1
            mask = None
        elif self.cls_mode == '1p_19q':
            mutation = np.array(df_from_excel['IDH_mutation']).astype(np.bool_) # IDH mutation status 0/1
            name_list = np.array(df_from_excel['Anony_ID']) # Patient ID
            cls_list = np.array(df_from_excel['1p/19q codeletion']) # 1p/19q codeletion status 0/1
            mask = mutation
