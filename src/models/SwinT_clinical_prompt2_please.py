...
class ClinicalSwinT(SwinTransformer):
    ...
    def __init__(self, img_size=224, in_chans=3, num_classes=1, window_size=7,
        ...
        self.mm_cls_token = nn.Parameter(torch.zeros(self.num_features))
        ...
        if self.clini:
            self.decoder_pos_embed = nn.Parameter(torch.randn(1, 4, self.num_features) * .02)
        else:
    ...
    def forward(self, x, clinical_feats):
        ...
        if self.fusion_style == 'self-attn':
            if self.clini:
                multi_modal_feats = torch.stack([self.mm_cls_token.expand(x.shape[0], -1), x, age_embed, sex_embed], 1)
                # ---
