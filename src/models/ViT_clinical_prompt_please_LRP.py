...
class ClinicalViT(VisionTransformer):
    ...
    def __init__(self, img_size=224, in_chans=3, num_classes=1,
        ...
        self.mm_cls_token = nn.Parameter(torch.zeros(embed_dim))
        nn.init.normal_(self.mm_cls_token, std=1e-6)
        
        # self.multimodal_cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        # nn.init.normal_(self.multimodal_cls_token, std=1e-6)
        
        self.decoder_pos_embed = nn.Parameter(torch.randn(1, 4, embed_dim) * .02)
    ...
    def forward(self, x, clinical_feats):
        ...
        x = self.norm(x)
        img_out = self.idxselect(x, dim=1, indices=torch.tensor(0, device=x.device))
        # ----------------------------------------------------------------------
        ...
        if self.fusion_style == 'self-attn':
            ...
            mm_cls_token = self.mm_cls_token.expand(img_out.shape[0], -1).unsqueeze(1)
            multi_modal_feats = self.cat((mm_cls_token, img_out, age_embed, sex_embed), 1)
            # # ---
    ...
    def relprop(self, cam=None, method="transformer_attribution", is_ablation=False, start_layer=0, **kwargs):
        ...
        cls_cam, img_cam, age_cam, sex_cam = self.cat.relprop(cam, **kwargs)
    ...
    def forward_clinical_embed(self, clinical_feats):
        ...
        if self.clini_info_style == 'bert':
            ...
            # clinical_prompt = ['a magnetic resonance image of ' + age[0] + ' patient', 'a magnetic resonance image of a ' + sex[0] + ' patient']
            # clinical_token = self.tokenizer(clinical_prompt, return_tensors="pt", padding=True, truncation=True)
            # clinical_token = {k: v.cuda() for k, v in clinical_token.items()}

            # self.embed_model.eval()
            # clinical_embed = self.embed_model(**clinical_token).last_hidden_state
            # clinical_embed = self.pool_clinical(clinical_embed, dim=1, indices=torch.tensor(self.clini_embed_token_idx, device=clinical_embed.device)).squeeze(1)
