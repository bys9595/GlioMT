...
class ClinicalSwinT(SwinTransformer):
    ...
    def forward_clinical_embed(self, clinical_feats):
        ...
        age = ['old' if int(i)>=self.age_cutoff else 'young' for i in age]

        if self.clini_info_style == 'bert':
            age_prompt = [age[i] for i in range(len(age))]
            sex_prompt = [sex[i] for i in range(len(sex))]
