...
from .vit_utils.layers_swint_lrp import *
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        # num_windows*B, N, 3*C -> 
        # num_windows*B, N, 3, self.num_heads, C//self.num_heads -> 
        # 3, num_windows*B, self.num_heads, N, C//self.num_heads
        q, k, v = qkv.unbind(0) # each      num_windows*B, self.num_heads, N, C//self.num_heads
        ...
        if self.fused_attn:
            ...
            q = q * self.scale
            attn = self.matmul1([q, k]) # attn = q @ k.transpose(-2, -1) # num_windows*B, self.num_heads, N, N
            # attn = self.add1([attn, self._get_rel_pos_bias()]) # 
            attn = attn + self._get_rel_pos_bias()
            if mask is not None:
                num_win = mask.shape[0]
                attn = attn.view(-1, num_win, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0)
                attn = attn.view(-1, self.num_heads, N, N)
            attn = self.softmax(attn) # num_windows*B, self.num_heads, N, N
            attn = self.attn_drop(attn) # num_windows*B, self.num_heads, N, N
            
            self.save_attn(attn)
            attn.register_hook(self.save_attn_gradients)
            
            x = self.matmul2([attn, v]) # x = attn @ v 
            
        # num_windows*B, self.num_heads, N, C//self.num_heads
        x = x.transpose(1, 2).reshape(B_, N, -1) 
        # num_windows*B, N, self.num_heads, C//self.num_heads -> num_windows*B, N, C
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
    ...
    def relprop(self, cam, **kwargs):
        ...
        cam = rearrange(cam, 'b n (h c) -> b h n c', h=self.num_heads)
        ...
        cam_qkv = rearrange([cam_q, cam_k, cam_v], 'qkv b h n c -> b n (qkv h c)', qkv=3, h=self.num_heads)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut, x = self.clone1(x, 2) # B, H, W, C
        x = self.norm1(x)
        ...
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C) # num_win*B, window_size, window_size, C
        ...
        # FFN
        x = self.add1([shortcut, self.drop_path(x)]) # B H' W' C
        x = x.reshape(B, -1, C) # B H*W C
        x1, x2 = self.clone2(x, 2)
        x = self.add2([x1, self.drop_path(self.mlp(self.norm2(x2)))]) # B H*W C
        x = x.reshape(B, H, W, C)
        return x
    
    def relprop(self, cam, **kwargs):
        H, W = cam.shape[1], cam.shape[2]
        cam = rearrange(cam, 'b h w c -> b (h w) c')
        (cam1, cam2) = self.add2.relprop(cam, **kwargs)
        cam2 = self.drop_path.relprop(cam2, **kwargs)
        cam2 = self.mlp.relprop(cam2, **kwargs)
        cam2 = self.norm2.relprop(cam2, **kwargs)
        cam = self.clone2.relprop((cam1, cam2), **kwargs)
        cam = rearrange(cam, 'b (h w) c -> b h w c', h=H, w=W)
        (shortcut_cam, cam) = self.add1.relprop(cam, **kwargs)
        cam = self.drop_path.relprop(cam, **kwargs)
        
        if self.shift_size > 0:
            cam = torch.roll(cam, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))

        # merge windows - reverse
        cam = rearrange(cam, 'b (h n) (w m) c -> b h n w m c', n=self.window_size, m=self.window_size)
        h, w = cam.shape[1], cam.shape[3]
        
        cam = rearrange(cam, 'b h n w m c -> b h w n m c')
        cam = rearrange(cam, 'b h w n m c -> (b h w) n m c')
        
        cam = rearrange(cam, 'b n m c -> b (n m) c', n=self.window_size, m=self.window_size)
        
        # W-MSA/SW-MSA - reverse
        cam = self.attn.relprop(cam, **kwargs)
        
        # partition windows -reverse
        cam = rearrange(cam, 'b (n m) c -> b n m c', n=self.window_size, m=self.window_size)
        
        cam = rearrange(cam, '(b h w) n m c -> b h w n m c', h=h, w=w)
        cam = rearrange(cam, 'b h w n m c -> b h n w m c')
        cam = rearrange(cam, 'b h n w m c -> b (h n) (w m) c')
        
        if self.shift_size > 0:
            cam = torch.roll(cam, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        
        cam = self.norm1.relprop(cam, **kwargs)
        cam = self.clone1.relprop((shortcut_cam, cam), **kwargs)
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, H // 2, 2, W // 2, 2, C).permute(0, 1, 3, 4, 2, 5).flatten(3) # (B, H//2, 2a, W//2, 2b, C) -> (B, H//2, W//2, 2b, 2a, C) -> (B, H//2, W//2, 4*C)
        x = self.norm(x)
        x = self.reduction(x)
        return x
    ...
    def relprop(self, cam, **kwargs):
        ...
        cam = rearrange(cam, 'b h w (n m c) -> b (h m) (w n) c', n=2, m=2)
...
class SwinTransformer(nn.Module):
    ...
    def relprop(self, cam=None,method="transformer_attribution", is_ablation=False, start_layer=0, **kwargs):
        ...
        cam = self.norm.relprop(cam, **kwargs)
        for lys in reversed(self.layers):
            cam = lys.relprop(cam, **kwargs)
        ...
        if method == "full":
            ...
            # cam rollout
            attn_cams = []
            for lys in self.layers:
                attn_heads = lys.blocks.attn.get_attn_cam().clamp(min=0)
        ...
        # our method, method name grad is legacy
            ...
            for lys in self.layers:
                for blk in lys.blocks:
                    ...
                    # x, w = int(cam.shape[0]**0.5), int(cam.shape[1]**0.5)
                    # cam = rearrange(cam, '(x1 x2) (w1 w2) -> (x1 w1) (x2 w2)', x1=x, x2=x, w1=w, w2=w)
                    # cam = torch.nn.functional.interpolate(cam.unsqueeze(0).unsqueeze(0), size=(H, W), mode='bilinear')[0]

                    cams.append(cam)
            rollouts = []
            for i in range(len(self.depths)):
                rollout = compute_rollout_attention(cams[sum(self.depths[:i]):sum(self.depths[:i+1])], start_layer=start_layer)
                rollout = rollout.mean(-1)
                x, w = int(rollout.shape[0]**0.5), int(rollout.shape[1]**0.5)
                rollout = rearrange(rollout, '(x1 x2) (w1 w2) -> (x1 w1) (x2 w2)', x1=x, x2=x, w1=w, w2=w)
                rollouts.append(rollout)
            # rollout = compute_rollout_attention(cams, start_layer=start_layer)
            # cam = rollout.unsqueeze(0)
            # cam = cams[0].mean(-1)
...
if __name__ == "__main__":
    ...
    net = create_model(model_name='swin_small_patch4_window7_224', pretrained=True, pretrained_cfg='ms_in1k', in_chans=4, num_classes=1)
    net = net.cuda()
    
    img = torch.randn((6, 4, 224, 224)).cuda()
