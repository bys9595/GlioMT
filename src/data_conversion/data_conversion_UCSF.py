...
def conversion(patient_name, data_dir, seg_dir, out_dir):
    # Read seg mask
    seg_dir = os.path.join(seg_dir, patient_name)
    seg_mask_itk =  sitk.ReadImage(os.path.join(seg_dir, patient_name.split('_')[0] +'_tumor_segmentation.nii.gz'))
    seg_mask =  sitk.GetArrayFromImage(seg_mask_itk)
    
    # Read Image
    # print(os.path.join(data_dir, patient_name, patient_name.split('_')[0]+'_T1_bias.nii.gz'))
    
    T1_itk =  sitk.ReadImage(os.path.join(data_dir, patient_name, patient_name.split('_')[0]+'_T1_bias.nii.gz'))
    T1C_itk =  sitk.ReadImage(os.path.join(data_dir, patient_name, patient_name.split('_')[0]+'_T1c_bias.nii.gz'))
    T2_itk =  sitk.ReadImage(os.path.join(data_dir, patient_name, patient_name.split('_')[0]+'_T2_bias.nii.gz'))
    FLAIR_itk =  sitk.ReadImage(os.path.join(data_dir, patient_name, patient_name.split('_')[0]+'_FLAIR.nii.gz'))
    ...
    if os.path.isfile(os.path.join(data_dir, patient_name, patient_name.split('_')[0]+'_ADC.nii.gz')):
        ADC_itk =  sitk.ReadImage(os.path.join(data_dir, patient_name, patient_name.split('_')[0]+'_ADC.nii.gz'))
...
if __name__ == "__main__":
    data_dir = "/mnt/BYS/dataset/UCSF_original/"
    # seg_dir = "/mai_nas/BYS/brain_metastasis/nnunet/nnunet_results/SEV"
    seg_dir = data_dir
    out_dir = "/mnt/BYS/dataset/UCSF/"
    # --------------------------------------------------------------------------------------
